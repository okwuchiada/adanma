...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    const pointerInstance = new (pointerPrototype as any)();
    let pointers: Pointer[] = [pointerInstance];
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): GLContext {
      ...
      if (isWebGL2) {
        ...
        const halfFloatExt = (gl as WebGLRenderingContext).getExtension(
          "OES_texture_half_float"
        );
        halfFloatTexType = halfFloatExt?.HALF_FLOAT_OES ?? 0x8d61; // Fallback to FLOAT if half-float not available
      ...
      if (isWebGL2) {
        ...
        // In WebGL1, we can only reliably use RGBA format
    ...
    function getSupportedFormat(
      ...
      if (!supportRenderTextureFormat(gl, internalFormat, format, type)) {
        // Handle fallback formats differently for WebGL2 vs WebGL1
        if ("R16F" in gl) {
          // WebGL2 path
          const gl2 = gl as WebGL2RenderingContext;
          ...
          // WebGL1 path - just use RGBA format
          const gl1 = gl as WebGLRenderingContext;
          return { internalFormat: gl1.RGBA, format: gl1.RGBA };
        }
      }
    ...
    class Material {
      ...
      uniforms: Record<string, WebGLUniformLocation | null>; // Changed from array to object

      constructor(vertexShader: WebGLShader, fragmentShaderSource: string) {
        this.vertexShader = vertexShader;
        this.fragmentShaderSource = fragmentShaderSource;
        this.programs = {}; // Initialize as empty object
    ...
    const advectionShader = compileShader(
      ...
      `
        ...
        `,
      ext.supportLinearFiltering ? undefined : ["MANUAL_FILTERING"] // Changed null to undefined
    );
    ...
    function resizeCanvas(): boolean {
      if (!canvas) return false; // Early return if canvas is null
